...
public class ConversionUnidades extends javax.swing.JDialog {
    public ConversionUnidades(java.awt.Frame parent, boolean modal) {
        super(parent, modal);
        initComponents();
        sessionHibernate();
        arranque();
        setLocationRelativeTo(null);
        this.setTitle("Conversion de Unidades de Medida - KARU v1.0");
        this.usuario = "karu";
        this.formulario = this.getClass().getSimpleName();        
    }
    
    public ConversionUnidades(String user) {
        initComponents();
        sessionHibernate();
        arranque();
        setLocationRelativeTo(null);
        ...
        jPanel1Layout.setHorizontalGroup(
            ...
            .addGroup(jPanel1Layout.createSequentialGroup()
                ...
                .addGroup(jPanel1Layout.createParallelGroup(javax.swing.GroupLayout.Alignment.TRAILING)
                    .addComponent(lblComentario, javax.swing.GroupLayout.PREFERRED_SIZE, 509, javax.swing.GroupLayout.PREFERRED_SIZE)
                    ...
                    .addGroup(javax.swing.GroupLayout.Alignment.LEADING, jPanel1Layout.createSequentialGroup()
                        ...
                        .addGap(18, 18, 18)
                        .addComponent(txtFactor, javax.swing.GroupLayout.PREFERRED_SIZE, 120, javax.swing.GroupLayout.PREFERRED_SIZE)))
    ...
    private void txtFactorFocusLost(java.awt.event.FocusEvent evt) {//GEN-FIRST:event_txtFactorFocusLost
        try {
            this.txtFactor.commitEdit();
            this.lblComentario.setText("1 " + this.cmbOrigen.getSelectedItem().toString() + " del origen equivale a " + 
                    this.txtFactor.getValue().toString() + " " + this.cmbDestino.getSelectedItem().toString() + " del destino.");
        } catch (Exception e){
        }
    ...
    public static void main(String args[]) {
        ...
        try {
            ...
            java.util.logging.Logger.getLogger(ConversionUnidades.class.getName()).log(java.util.logging.Level.SEVERE, null, ex);
        ...
        java.awt.EventQueue.invokeLater(new Runnable() {
            public void run() {
                ConversionUnidades dialog = new ConversionUnidades(new javax.swing.JFrame(), true);
    ...
    private void arranque(){
        ...
        this.lblComentario.setText("Ingrese unidad");
        this.cmbOrigen.setSelectedIndex(-1);
        this.cmbDestino.setSelectedIndex(-1);
        this.cmbOrigen.setEnabled(false);
        this.cmbDestino.setEnabled(false);
        this.txtFactor.setValue(null);
    ...
    public void cargarTabla(){
        List<MtConversionUnidades> lista;
        lista = (List<MtConversionUnidades>)st.createQuery("From MtConversionUnidades").list();
        for (MtConversionUnidades laLista : lista) {
            model.addRow(new Object[]{
                retornoUnidMedidaPorCod(laLista.getId().getCodUniOri()),
                retornoUnidMedidaPorCod(laLista.getId().getCodUniDes()),
                laLista.getFactor()
            });
        }
    }
    ...
    public void nuevo(){
        ...
        //Setea el estado de los componentes editables (textbox, combobox, etc)
        this.cmbOrigen.setEnabled(true);
        this.cmbOrigen.setSelectedIndex(-1);
        this.cmbDestino.setEnabled(true);
        this.cmbDestino.setSelectedIndex(-1);
        this.txtFactor.setEditable(true);
    ...
    public void editar(){
        ...
        this.txtFactor.requestFocusInWindow();
    }
    
    public void eliminar() {
        int seleccion = JOptionPane.showConfirmDialog(null, "Desea eliminar el Registro?.", "Confirmacion", JOptionPane.YES_NO_OPTION);
        ...
        if (seleccion == 1) {
            ...
            try {
                st.beginTransaction();
                MtUnidMedidas origen = retornoUnidMedidaPorDesc(this.cmbOrigen.getSelectedItem().toString());
                MtUnidMedidas destino = retornoUnidMedidaPorDesc(this.cmbDestino.getSelectedItem().toString());

                MtConversionUnidadesId id = new MtConversionUnidadesId(origen.getCodUnidad(), destino.getCodUnidad());
                MtConversionUnidades cabecera = (MtConversionUnidades) st.load(MtConversionUnidades.class, id);
    ...
    private void cancelar() {
        int seleccion = JOptionPane.showConfirmDialog(null, "¿Desea cancelar la operación?.", "Confirmacion", JOptionPane.YES_NO_OPTION);
        if (seleccion == 1) {
        } else {
            arranque();
        }
    ...
    public void guardar() {
        if (this.cmbOrigen.getSelectedIndex() == -1 || this.cmbDestino.getSelectedIndex() == -1) {
            JOptionPane.showMessageDialog(null, "Debe seleccionar el origen y el destino.");
        } else if (this.cmbOrigen.getSelectedItem().equals(this.cmbDestino.getSelectedItem())) {
            JOptionPane.showMessageDialog(null, "El origen y el destino no pueden ser iguales.");
        } else if (this.txtFactor.getValue() == null || Double.parseDouble(this.txtFactor.getValue().toString()) <= 0.0) {
            JOptionPane.showMessageDialog(null, "El factor debe ser mayor a 0.");
        } else {
            st.beginTransaction();
            MtUnidMedidas origen = retornoUnidMedidaPorDesc(this.cmbOrigen.getSelectedItem().toString());
            MtUnidMedidas destino = retornoUnidMedidaPorDesc(this.cmbDestino.getSelectedItem().toString());
            ...
            MtConversionUnidades cabecera = new MtConversionUnidades();
            cabecera.setId(id);
            cabecera.setFactor(BigDecimal.valueOf(Double.parseDouble(this.txtFactor.getValue().toString())));
    ...
    public void guardarEd() {
        if (this.txtFactor.getValue() == null || Double.parseDouble(this.txtFactor.getValue().toString()) <= 0.0) {
            JOptionPane.showMessageDialog(null, "El factor debe ser mayor a 0.");
        } else {
            st.beginTransaction();
            MtUnidMedidas origen = retornoUnidMedidaPorDesc(this.cmbOrigen.getSelectedItem().toString());
            ...
            try {
                ...
                JOptionPane.showMessageDialog(this, "Ha ocurrido un error al guardar el registro." + "\n"
    ...
    public void obtenerTabla(){
        ...
        Object factor = model.getValueAt(selectedRow, 2);
        this.cmbOrigen.setSelectedItem(origen);
        this.cmbDestino.setSelectedItem(destino);
        this.txtFactor.setValue(factor);
        this.txtFactorFocusLost(null);
    ...
    public void cargarComboOrigen(){
        ...
        List<MtUnidMedidas> origenes = (List<MtUnidMedidas>)st.createQuery("From MtUnidMedidas").list();
        for(MtUnidMedidas origen : origenes){
            this.cmbOrigen.addItem(origen.getDescUnidad());
        }
    }
    
    public void cargarComboDestino(){
        this.cmbDestino.removeAllItems();
        List<MtUnidMedidas> destinos = (List<MtUnidMedidas>)st.createQuery("From MtUnidMedidas").list();
        for(MtUnidMedidas destino : destinos){
            this.cmbDestino.addItem(destino.getDescUnidad());
        }
    }    
    
    public MtUnidMedidas retornoUnidMedidaPorDesc(String descripcion) {
    ...
    public String retornoUnidMedidaPorCod(String codigo) {
        MtUnidMedidas tipRet;
        String descripcion = "";
        try {
            Query query = st.createQuery("From MtUnidMedidas c Where c.codUnidad = ?");
            query.setParameter(0, codigo);
            try {
                tipRet = (MtUnidMedidas) query.uniqueResult();
                descripcion = tipRet.getDescUnidad();
            } catch (Exception e) {
                JOptionPane.showMessageDialog(null, "Hay más cargos de tipo: " + codigo);
            }
        } catch (Exception e) {
